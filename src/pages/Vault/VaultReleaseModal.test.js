...
const sampleRelease = {
  ...
  release_art: "https://bng-resources.s3.us-east-2.amazonaws.com/7/art/Trials%20And%20Errors.jpg",
  release_description:
...
describe("Release content renders correctly in modal", () => {
  ...
  test("Release content is in the modal", () => {
    expect(screen.getByRole("heading", { name: /trials and errors/i })).toBeInTheDocument();
  });

  test("Artist name is in the modal", () => {
    expect(screen.getByRole("heading", { name: /lingero/i })).toBeInTheDocument();
  });
